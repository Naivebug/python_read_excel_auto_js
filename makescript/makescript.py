...
def TestPrint(*msg):
    strmsg = ''
    for temmsg in msg:
        if isinstance(temmsg,unicode):
            temmsg = GetEncodeStr(temmsg) 
        else:
            temmsg = str(temmsg)
...
def LuaReadData2Map(fname,sheetName,title,string,tuples,dataName,noReadRow = 2):
    data = ReadData2Map(fname,sheetName,title,string,tuples,dataName,noReadRow = 2);
    exec data;
    data = slpp.SLPP().encode(eval(dataName));
    data = "\n--%s\n" % (str(title)) + dataName + "=" + data# + "\n"  + MapDataProductLuaFunctionGetData(dataName,title) 
...
#make to app file
def MakeData2FileApp(body,pathout,dataName,AddFlag3 = "",AddHead = ""):
    #import json
    exec body
    #body = eval(dataName)
    #body = json.dumps(body,indent=4,sort_keys=True,separators = (', ', ': '))
    pathout = g_PathOut + pathout
    MakeDir(pathout);    #
    strtem = body;
    f = open(pathout,'w+');
...
#make to app file
def MakeData2FileTS(body,pathout,dataName,AddFlag3 = "",AddHead = ""):
    import json
    exec body
    #body = eval(body)
    #body = json.dumps(body,indent=4,sort_keys=True,separators = (', ', ': '))
    pathout = g_PathOut + pathout
    MakeDir(pathout);    #
    if(g_types.get('module')) :
        strtem = "module mkdata { \n\texport var "+ dataName + " =" +  body + "\n}"
    else :
        strtem = "export var "+ dataName + " =" +  body + "\n"
    f = open(pathout,'w+');
    f.write(strtem);
    f.close();
    return
...
# 字典数据生成Lua直接获得的函数
def MapDataProductLuaFunctionGetData(key,ls):
    s = ''
    newline = "\n"
    tab = "\t"
    for i ,string in enumerate(ls):
        s += newline
        if i == 0:
            s += "function %s.GetData(id,default)" % key + newline
            s += tab + "if %s[id] then return %s[id] else return default end " %(key, key) + newline
            s += "end" + newline
        else:
            s += "function %s.Get%s(data,default)" % (key,string) + newline
            s += tab + "if data[%d] then return data[%d] else return default end " %( i,i) + newline
            s += "end" + newline
            s += "function %s.Get%sFormID(id,default)" % (key,string) + newline
            s += tab + "if %s.GetData(id) then return %s.Get%s(%s.GetData(id)) else return default  end" % (key,key,string,key) + newline
            s += "end" + newline
    return s

def ChangeTitleData2Class(dataName,title):
    temstr = "";
    newline = "\n"
    tab = "\t"
    global  g_nCen;
    ...
    def GetTitleStr(temtitle,ncen):
        ...
        for ncur,keys in enumerate(temtitle):
            i = ncur
            if (type(keys) == tuple or  type(keys) == list):  # 子集
                strline += "function %s.Get%s(data,default)" % (key,keys[0]) + newline
                strline += tab + "if data[%d] then return data[%d] else return default end " %(i, i) + newline
                strline += "end" + newline
                strline += "function %s.Get%sData(data,id,default)" % (key,keys[0]) + newline
                strline += tab + "if data[%d][id] then return data[%d][id] else return default end " %(i, i) + newline
                strline += "end" + newline
                strline += GetTitleStr(keys,ncen + 1);
            else:
                string = keys
                if ncur == 0:
                    if ncen == 0:
                        strline += "function %s.GetData(id,default)" % key + newline
                        strline += tab + "if  %s['data'][id] then return %s['data'][id] else return default end " % (key,key) + newline
                        strline += "end" + newline
                else:
                    strline += "function %s.Get%s(data,default)" % (key,string) + newline
                    strline += tab + "if data[%d] then return data[%d] else return default end  " % (i,i) + newline
                    strline += "end" + newline
                    strline += "function %s.Get%sFormID(id,default)" % (key,string) + newline
                    strline += tab + "if %s.GetData(id) then return %s.Get%s(%s.GetData(id)) else return default  end" % (key,key,string,key) + newline
                    strline += "end" + newline
        return strline 
    temstr += GetTitleStr(title,0)
    return temstr
...
def ChangeTitleDictData2Class(dataName,title):
    temstr = "";
    newline = "\n"
    tab = "\t"
    global  g_nCen;
    ...
    def GetTitleStr(temtitle,ncen):
        ...
        for ncur,keys in enumerate(temtitle):
            ...
            if (type(keys) == tuple or  type(keys) == list):  # 子集
                strline += "function %s.Get%s(data,default)" % (key,keys[0]) + newline
                strline += tab + "if data['%s'] then return data['%s'] else return default end " %(keys[0], keys[0]) + newline
                strline += "end" + newline
                strline += "function %s.Get%sData(data,id,default)" % (key,keys[0]) + newline
                strline += tab + "if data['%s'][id] then return data['%s'][id] else return default end " %(keys[0], keys[0]) + newline
                strline += "end" + newline
                strline += GetTitleStr(keys,ncen + 1);
            else:
                string = keys
                if ncur == 0:
                    ...
                    strline += "function %s.Get%s(data,default)" % (key,string) + newline
                    strline += tab + "if data['%s'] then return data['%s'] else return default end  " % (string,string) + newline
                    strline += "end" + newline
                    strline += "function %s.Get%sFormID(id,default)" % (key,string) + newline
                    strline += tab + "if %s.GetData(id) then return %s.Get%s(%s.GetData(id)) else return default  end" % (key,key,string,key) + newline
                    strline += "end" + newline
...
def UseTest1():
    fname = "sample.xls"
    sheetName = "Sheet1";
    title = ("ID" ,"Name" ,"Tuple","Type");
    string = ("Name",);
    tuples = ("Tuple",);
    # data = ReadTest1(fname,sheetName,title,string,tuple,noReadRow = 2);
    dataName = "DataMap"
    data = ReadData2Map(fname,sheetName,title,string,tuples,dataName,noReadRow = 2);
    MakeData2File(data,'data/test.py',dataName,dataName);
    dataName = "DataMap1"
    MakeData2File(data,'data/test.py',dataName,dataName);
    # return
    
    title = ("ID" ,"Name" ,("Type","Tuple",));
    string = ("Name",);
    tuples = ("Tuple",);
    sheetName = "Sheet2";
    # data = ReadTest1(fname,sheetName,title,string,tuple,noReadRow = 2);
    dataName = "DataTupe"
    data = ReadData2MapList(fname,sheetName,title,string,tuples,dataName,noReadRow = 2);
    MakeData2File(data,'data/test.py',dataName,dataName);
    return
########################################################################
###########################################################################
...
def ReadTitle2Dict(fname, sheetName, title, string, tuples,floats, dataName, noReadRow=2):
    ...
    nrows = sh.nrows
    temstr = "";
    if dataName == "" : # App类型不需要DataName,直接写进去
        temstr +="{"
    else:
        temstr += dataName + "= {"
    keyid = title[0];  # 第一个默认为关键字
