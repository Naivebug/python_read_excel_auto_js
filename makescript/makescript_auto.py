...
def Auto():
    ...
    for name in dirs:                             
        if (os.path.splitext(name)[1] == ".xlsx" or os.path.splitext(name)[1] == ".xlsm") and name[0] != "~":  
            TestPrint("name",name)
            AutoDaoBiao(name)
...
def AutoDaoBiaoSheet(fname,sheetName,sh):
    ...
    for j in range(0,sh.ncols):
        ...
        stype3 =  int(GetCellValue(sh,2,j) or 0) 
    ...
    if _temGroupV == 1:
        ...
        ls = []
        ls2 = ls
        print ("---temgrounp",_temGroup)
        for m in range(2,10):
            if(_temGroup.has_key(m)):
                v = _temGroup[m]
                if(len(ls)==0):
                    ls = v;
                    ls2 =v
    ...
    TestPrint("开始分页:", fname, sheetName)
    ...
    if(len(titleS) > 0 ):
        ...
        if _temGroupV==1:
            data = ReadTitleDictList(fname,sheetName,titleS,stringS,tuplesS,floatsS,"",noReadRow = 5);
        elif _temGroupV == 2:
            data = ReadTitleDictDictDict(fname,sheetName,titleS,stringS,tuplesS,floatsS,"",noReadRow = 5);
        else:
            data = ReadTitle2Dict(fname,sheetName,titleS,stringS,tuplesS,floatsS,"",noReadRow = 5);
